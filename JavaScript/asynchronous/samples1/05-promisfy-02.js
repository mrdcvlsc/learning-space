...
function PromiseGetLoginInfo(Username, Password){
...
function PromiseAuthenticate(Username, Password){
  console.log('Verifying login credentials...');
  return new Promise((resolve,reject)=>{
    setTimeout(()=>{
      let status = {};
...
// using then
// PromiseGetLoginInfo('Admins','admin123')
// .then(PromiseAuthenticate)
// .then(GetStatus);

// using async and await
async function Sequence(){
  let LoginInfo = await PromiseGetLoginInfo('Admin','admin123');
  let Auth = await PromiseAuthenticate(LoginInfo);
  GetStatus(Auth);
};
Sequence();